      ...
      children: [
        ...
        Row(
          ...
          children: [
            ...
            if (hhInfo.archives.length >= 6) _HHDownloadButtonSet(archive: hhInfo.archives[5])
          ],
        ),
      ],
    );
  ...
  Future<void> _getHHInfo() async {
    ...
    try {
      hhInfo = await ehRequest.get(
          url: widget.archivePageUrl, parser: EHSpiderParser.archivePage2HHInfo);
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ElevatedButton(
          onPressed:
              archive.resolution == null ? null : () => backRoute(result: archive.resolution),
          child: SizedBox(
            width: UIConfig.hhDialogTextButtonWidth,
            child: Center(
              child: Text(archive.resolutionDesc,
                  style: const TextStyle(fontSize: UIConfig.archiveDialogDownloadTextSize)),
            ),
          ),
        ),
        Row(
