...
Future<T?>? toRoute<T>(
  ...
  if (page.side == Side.left) {
    if (styleSetting.layout.value == LayoutMode.desktop) {
      ...
      int? tabIndex =
          logic.state.icons.firstIndexWhereOrNull((icon) => icon.routeName == routeName);
      if (tabIndex != null) {
...
void backRoute<T>({
  ...
  Side side =
      Routes.pages.firstWhereOrNull((page) => page.name == currentRoute)?.side ?? Side.fullScreen;
...
Future<T?>? offRoute<T>(
  ...
  Side side =
      Routes.pages.firstWhereOrNull((page) => page.name == routeName)?.side ?? Side.fullScreen;
