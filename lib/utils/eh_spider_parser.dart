...
import 'package:jhentai/model/gallery_torrent.dart';
...
class EHSpiderParser {
  ...
  static ({
    ...
    String? thumbnailSize =
        document.querySelector('#tssel > div > label > input[checked=checked]')?.parent?.text;
    bool isLargeThumbnail =
  ...
  static ({bool isDonator, int? currentConsumption, int? totalLimit, int? resetCost})
  ...
  static GalleryArchive archivePage2Archive(Headers headers, dynamic data) {
    ...
    return GalleryArchive(
      ...
      originalCost:
          document.querySelector('#db > div > div > div > strong')!.text.replaceAll(',', ''),
      originalSize: document.querySelector('#db > div > div > p > strong')!.text,
      downloadOriginalHint:
          document.querySelector('#db > div > div > form > div > input')!.attributes['value']!,
  ...
  static Gallery _parseMinimalGallery(Element tr) {
    ...
    Gallery gallery = Gallery(
      ...
      hasRated: tr.querySelector('.gl4m > .ir')!.attributes['class']!.split(' ').length > 1
          ? true
          : false,
      favoriteTagIndex: _parseMinimalGalleryFavoriteTagIndex(tr),
      favoriteTagName:
          tr.querySelector('.gl2m > div:nth-child(2) > [id][style]')?.attributes['title'],
  ...
  static Gallery _parseCompactGallery(Element tr) {
    ...
    Gallery gallery = Gallery(
      ...
      favoriteTagName:
          tr.querySelector('.gl2c > div:nth-child(2) > [id][style]')?.attributes['title'],
      tags: tags,
  ...
  static Gallery _parseExtendedGallery(Element tr) {
    ...
    Gallery gallery = Gallery(
      ...
      rating: _parseGalleryRating(tr),
      hasRated: tr.querySelector('.gl3e > .ir')!.attributes['class']!.split(' ').length > 1
          ? true
          : false,
  ...
  static List<VisitStat> _parseStats(Element tbody) {
    List<String> periods =
        tbody.querySelectorAll('tr:nth-child(4) > .stdk').map((e) => e.text).toList();
    List<String> visits =
        tbody.querySelectorAll('tr:nth-child(6) > .stdv').map((e) => e.text).toList();
    List<String> hits =
        tbody.querySelectorAll('tr:nth-child(8) > .stdv').map((e) => e.text).toList();
