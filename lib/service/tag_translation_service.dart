...
class TagTranslationService with JHLifeCircleBeanErrorCatch implements JHLifeCircleBean {
  ...
  Future<List<TagData>> translateTagDatasIfNeeded(List<TagData> tags) async {
    ...
    List<Future<TagData>> futures = tags
        .map((tag) => getTagTranslation(tag.namespace, tag.key).then((value) => value ?? tag))
        .toList();
  ...
  Future<List<TagAutoCompletionMatch>> searchTags(String searchText, {int? limit}) async {
    ...
    List<({String? sNamespace, String sKey, int matchStart, int matchEnd})> searchList =
        matches.map((match) {
    ...
    for (int i = 0; i < searchList.length; i++) {
      String searchTextMerged = searchList
          .sublist(i)
          .map((e) => e.sNamespace != null ? '${e.sNamespace}:${e.sKey}' : e.sKey)
          .join(' ');
      ...
      if (tagSearchOrderOptimizationService.isReady) {
        List<TagData> tagDatas = sNameSpaceMerged != null
            ? await TagDao.searchFullTags(sNameSpaceMerged, '%$sKeyMerged%')
            : await TagDao.searchTags('%$sKeyMerged%');
        matches = await _markTagDatasByFrequency(searchText, searchList[i].matchStart,
            searchList[i].matchEnd, sNameSpaceMerged, sKeyMerged, tagDatas);
      } else {
        List<TagData> tagDatas = sNameSpaceMerged != null
            ? await TagDao.searchFullTagsIncludeIntro(sNameSpaceMerged, '%$sKeyMerged%')
            : await TagDao.searchTagsIncludeIntro('%$sKeyMerged%');
        matches = await _markTagDatasByScore(searchText, searchList[i].matchStart,
            searchList[i].matchEnd, sNameSpaceMerged, sKeyMerged, tagDatas);
      }
  ...
  Future<List<TagAutoCompletionMatch>> _markTagDatasByFrequency(String searchText, int matchStart,
    ...
    Map<TagData, int> tagCountMap = tagDatas.fold({}, (Map<TagData, int> map, tag) {
      map[tag] = tagCountDatas
              .firstWhereOrNull(
                  (tagCount) => tagCount.namespaceWithKey == '${tag.namespace}:${tag.key}')
              ?.count ??
          0;
      return map;
    });
  ...
  Future<List<TagAutoCompletionMatch>> _markTagDatasByScore(String searchText, int matchStart,
      int matchEnd, String? sNamespace, String sKey, List<TagData> tagDatas) async {
