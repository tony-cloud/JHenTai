...
class FirstOpenHandler implements UpdateHandler {
  ...
  Future<bool> match(int? fromVersion, int toVersion) async {
    return fromVersion == null ||
        (await localConfigService.read(configKey: ConfigEnum.firstOpenInited) == null);
  }
...
class MigrateSearchConfigHandler implements UpdateHandler {
  ...
  Future<void> onReady() async {
    ...
    Map<String, dynamic>? map =
        storageService.read('${ConfigEnum.searchConfig.key}: DesktopSearchPageTabLogic') ??
  ...
  Future<bool> match(int? fromVersion, int toVersion) async {
    if (fromVersion == null) {
      ...
      return fromVersion <= 10 ||
          (await localConfigService.read(configKey: ConfigEnum.migrateGalleryHistory) == null);
    }
  }
...
class MigrateStorageConfigHandler implements UpdateHandler {
  ...
  Future<bool> match(int? fromVersion, int toVersion) async {
    if (fromVersion == null) {
      ...
      return fromVersion <= 11 ||
          (await localConfigService.read(configKey: ConfigEnum.migrateStorageConfig) == null);
    }
  }
  ...
  Future<void> onInit() async {
    ...
    List<String>? cookies =
        storageService.read<List?>(ConfigEnum.ehCookie.key)?.cast<String>().toList();
    if (cookies != null) {
