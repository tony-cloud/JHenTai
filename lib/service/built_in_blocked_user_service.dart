...
class BuiltInBlockedUserService
    with JHLifeCircleBeanWithConfigStorage
    implements JHLifeCircleBean, ExtraBlockRuleProvider {
  ...
  List<JHLifeCircleBean> get initDependencies =>
      super.initDependencies..addAll([localBlockRuleService]);
  ...
  void applyBeanConfig(String configString) {
    ...
    if (list.isNotEmpty) {
      _blockedUsers.value =
          list.map((map) => (userId: map['userId'] as int, name: map['name'] as String)).toList();
    }
  }
  ...
  String toConfigString() {
    return jsonEncode(
        _blockedUsers.map((user) => {'userId': user.userId, 'name': user.name}).toList());
  }
  ...
  Future<void> doAfterBeanReady() async {
    ...
    List<EHUser> users = (map['blockedUsers'] as List)
        .map((map) => (userId: map['userId'] as int, name: map['name'] as String))
        .toList();

    log.info(
        'Built-in blocked user data loaded, version: $version, formatVersion: $formatVersion, updateTime: $updateTime, user count: ${users.length}');
  ...
  Map<String, List<LocalBlockRule>> get extraGroupedRules =>
      preferenceSetting.useBuiltInBlockedUsers.isTrue
          ? {
