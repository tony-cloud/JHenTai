...
class SuperResolutionService extends GetxController
  ...
  Future<void> deleteSuperResolve(int gid, SuperResolutionType type) async {
    ...
    if (type == SuperResolutionType.gallery) {
      ...
      dirPath = join(archiveDownloadService.computeArchiveUnpackingPath(archive.title, archive.gid),
          imageDirName);
    }
  ...
  Future<void> copyImageInfo(GalleryDownloadedData oldGallery, GalleryDownloadedData newGallery,
      int oldImageSerialNo, int newImageSerialNo) async {
    ...
    if (newGallerySuperResolutionInfo == null) {
      newGallerySuperResolutionInfo = SuperResolutionInfo(
        ...
        List.generate(galleryDownloadService.galleryDownloadInfos[newGallery.gid]!.images.length,
            (_) => SuperResolutionStatus.running),
      );
      superResolutionInfoTable.put(
          newGallery.gid, SuperResolutionType.gallery, newGallerySuperResolutionInfo);
