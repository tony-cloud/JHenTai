...
class VirtualFileOperations extends FileOperations {
  ...
  VirtualFileOperations(
    ...
    if (p.isAbsolute(startingDirectory)) {
      ...
      initialCurrentDirectory = p.normalize(p.join(rootDirectory, startingDirectory));
    }
    // --- End of changes ---
  ...
  String resolvePath(String path) {
    ...
    final absoluteVirtualPath =
        p.isAbsolute(cleanPath) ? cleanPath : p.normalize(p.join(currentDirectory, cleanPath));
  ...
  String _virtualToPhysical(String virtualPath) {
    ...
    final physicalPath = p.normalize(p.joinAll([physicalBase, ...remainingParts]));

    // Security check: Ensure the resolved physical path is *within* or *equal to*
    // its corresponding physical base directory.
    if (!p.isWithin(physicalBase, physicalPath) && !p.equals(physicalBase, physicalPath)) {
      throw FileSystemException(
  ...
  String? _tryDirectPhysicalResolution(String relativePath) {
    ...
    for (final entry in directoryMappings.entries) {
      ...
      final potentialPath = p.normalize(p.join(physicalBase, normalizedRelativePath));
      ...
      if ((p.isWithin(physicalBase, potentialPath) || p.equals(physicalBase, potentialPath)) &&
          (FileSystemEntity.typeSync(potentialPath) != FileSystemEntityType.notFound ||
  ...
  String _getVirtualPath(String physicalPath) {
    ...
    final entry = directoryMappings.entries.firstWhere(
      (e) => p.isWithin(e.value, physicalPath) || p.equals(e.value, physicalPath),
  ...
  void changeDirectory(String path) {
    try {
      ...
      if (!dir.existsSync()) {
        throw FileSystemException(
            "Directory not found: '$path' (resolved to '$targetPhysicalPath')", path);
      }
      ...
      throw FileSystemException("Failed to change directory to '$path': ${e.toString()}", path);
    }
  }
    ...
    try {
      ...
      if (resolvedPhysicalPath == rootDirectory) {
        ...
        return directoryMappings.keys
            .map(Directory.new) // Represent virtual entry using its key.
            .toList();
      }
      ...
      if (!await dir.exists()) {
        throw FileSystemException(
            "Directory not found: '$path' (resolved to '$resolvedPhysicalPath')", path);
      }
      ...
      throw FileSystemException("Failed to list directory '$path': ${e.toString()}", path);
    }
  }
  ...
  bool _isDirectChildOfRoot(String path) {
    final virtualPath = p.normalize(p.isAbsolute(path) ? path : p.join(currentDirectory, path));
  ...
  Future<void> writeFile(String path, List<int> data) async {
    if (_isDirectChildOfRoot(path)) {
      throw FileSystemException("Cannot create or write file directly in the virtual root", path);
    }
    final fullPath = resolvePath(path);
    if (fullPath == rootDirectory) {
  ...
  Future<List<int>> readFile(String path) async {
    final file = await getFile(path); // getFile already resolves and handles root
  ...
  Future<void> createDirectory(String path) async {
    if (_isDirectChildOfRoot(path)) {
      throw FileSystemException("Cannot create directory directly in the virtual root", path);
    }
    try {
      final fullPath = resolvePath(path);
      if (fullPath == rootDirectory) {
      ...
      throw FileSystemException("Failed to create directory '$path': ${e.toString()}", path);
    }
  }
  ...
  Future<void> deleteFile(String path) async {
    if (_isDirectChildOfRoot(path)) {
      throw FileSystemException("Cannot delete file directly in the virtual root", path);
    }
    final file = await getFile(path); // getFile already resolves and handles root
    if (await file.exists()) {
      await file.delete();
  ...
  Future<void> deleteDirectory(String path) async {
    if (_isDirectChildOfRoot(path)) {
      throw FileSystemException("Cannot delete directory directly in the virtual root", path);
    }
    final fullPath = resolvePath(path);
    if (fullPath == rootDirectory) {
    ...
    if (await dir.exists()) {
      ...
      throw FileSystemException("Directory not found for deletion: $path (resolved to $fullPath)");
    }
  }
  ...
  Future<int> fileSize(String path) async {
    final file = await getFile(path); // getFile already resolves and handles root
    if (!await file.exists()) {
      throw FileSystemException("File not found for size check: $path (resolved to ${file.path})");
    }
  ...
  bool exists(String path) {
    try {
      ...
      if (fullPath == rootDirectory) {
        return true; // Virtual root always exists
      }
  ...
  Future<void> renameFileOrDirectory(String oldPath, String newPath) async {
    ...
    if (_isDirectChildOfRoot(oldPath)) {
      throw FileSystemException(
          "Cannot rename file or directory directly in the virtual root", oldPath);
    }
    if (_isDirectChildOfRoot(newPath)) {
      throw FileSystemException("Cannot rename to a path directly in the virtual root", newPath);
    }
    ...
    if (sourceEntity == FileSystemEntityType.notFound) {
      throw FileSystemException("Source not found for rename: $oldPath (resolved to $oldFullPath)");
    }

    // Check if destination already exists
    if (FileSystemEntity.typeSync(newFullPath) != FileSystemEntityType.notFound) {
      throw FileSystemException("Destination already exists: $newPath (resolved to $newFullPath)");
    }
    ...
    final oldParts =
        p.split(oldVirtualPath).where((part) => part.isNotEmpty && part != rootDirectory).toList();
    final newParts =
        p.split(newVirtualPath).where((part) => part.isNotEmpty && part != rootDirectory).toList();

    if (oldParts.isNotEmpty && newParts.isNotEmpty && oldParts.first != newParts.first) {
      throw FileSystemException(
