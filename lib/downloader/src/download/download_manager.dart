...
class DownloadManager {
  ...
  int get currentBytes =>
      _chunks.fold(0, (previousValue, element) => previousValue + element.downloadedBytes);
  ...
  DownloadManager({
    ...
    adapter.createHttpClient = () {
      ...
      if (socksConfig != null) {
        ...
        _maybeWarnUnsupportedSocks();
        client.findProxy = (_) => 'DIRECT';
      } else {
        client.findProxy = ProxyConfig.toFindProxy(proxyConfig);
      }
      return client;
    };
  }
  ...
  SocksProxyConfiguration? _buildSocksConfig(ProxyConfig? config) {
    ...
    final SocksProxyConfiguration? parsed = parseSocksProxyConfiguration(
      config.address,
  ...
  void _maybeWarnUnsupportedSocks() {
    if (_warnedUnsupportedSocks) {
      return;
    }
  ...
  Future<void> changeIsolateCount(int count) async {
    ...
    await _statusChangeLock.lock(() async {
      ...
      if (_chunksReady &&
          _chunks.where((c) => !c.completed).length < _isolateCount &&
          !_chunks.every((chunk) => chunk.completed)) {
  ...
  void _handleChunkDownloadProgress(
      MainIsolateManager isolate, int chunkIndex, int newDownloadedBytes) {
  ...
  Future<void> _handleChunkDownloadError(
      MainIsolateManager isolate, int chunkIndex, JDownloadException error) async {
    await _killIsolates();
