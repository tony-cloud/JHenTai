...
import 'package:jhentai/pages/read/layout/horizontal_list/horizontal_list_layout_logic.dart';
...
class ReadPageLogic extends GetxController {
  ...
  void onReady() {
    ...
    toggleTurnPageByVolumeKeyLister =
        ever(readSetting.enablePageTurnByVolumeKeys, (_) => listen2VolumeKeys());

    /// Listen to immersive mode change
    toggleCurrentImmersiveModeLister =
        ever(readSetting.enableImmersiveMode, (_) => applyCurrentImmersiveMode());
  ...
  void applyCurrentBrightness() {
    if (GetPlatform.isMobile && readSetting.enableCustomReadBrightness.isTrue) {
      ScreenBrightness()
          .setApplicationScreenBrightness(readSetting.customBrightness.value.toDouble() / 100);
    }
  }
