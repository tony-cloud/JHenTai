...
class HorizontalListLayout extends BaseLayout {
  ...
  final HorizontalListLayoutLogic logic =
      Get.put<HorizontalListLayoutLogic>(HorizontalListLayoutLogic(), permanent: true);
      ...
      builder: (_, __) => JPhotoViewGalleryPageOptions.customChild(
        ...
        child: EHWheelSpeedControllerForReadPage(
          ...
          child: ScrollablePositionedList.separated(
            ...
            scrollOffsetController: state.scrollOffsetController,
            itemBuilder: (context, index) => readPageState.readPageInfo.mode == ReadMode.online
                ? buildItemInOnlineMode(context, index)
                : buildItemInLocalMode(context, index),
            separatorBuilder: (_, __) =>
                Obx(() => SizedBox(width: readSetting.imageSpace.value.toDouble())),
          ),
        ),
      ),
    );
