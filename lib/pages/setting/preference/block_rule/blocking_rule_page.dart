...
class BlockingRulePage extends StatelessWidget {
  ...
  Widget _elementBuilder(BuildContext context, String group, List<LocalBlockRule> rules) {
    return ListTile(
      minLeadingWidth: 60,
      leading: Text(rules.length == 1 ? rules.first.pattern.desc.tr : 'other'.tr,
  ...
  Widget _itemBuilder(BuildContext context, int index) {
    ...
    return ListTile(
      ...
      subtitle: Text(
        entry.value
            .map((rule) => '(${rule.attribute.desc.tr} ${rule.pattern.desc.tr} ${rule.expression})')
            .join(' && '),
        maxLines: 2,
        overflow: TextOverflow.ellipsis,
      ),
            ...
            child: Row(
              ...
              children: [
                ...
                SizedBox(
                    width: 56,
                    child:
                        Text('delete'.tr, style: TextStyle(color: UIConfig.alertColor(context)))),
              ],
            ),
            onPressed: () async {
              backRoute();
