...
class BlockingRulePageLogic extends GetxController {
  ...
  Future<void> onInit() async {
    ...
    String? showGroupString =
        await localConfigService.read(configKey: ConfigEnum.displayBlockingRulesGroup);
  ...
  Future<void> toggleShowGroup() async {
    ...
    await localConfigService.write(
        configKey: ConfigEnum.displayBlockingRulesGroup, value: state.showGroup.toString());
  }
  ...
  Future<void> getBlockRules() async {
    ...
    keys.sort((a, b) {
      int code =
          state.groupedRules[a]!.first.target.code - state.groupedRules[b]!.first.target.code;
      if (code != 0) {
        return code;
      }
  ...
  Future<void> removeLocalBlockRulesByGroupId(String groupId) async {
    ({bool success, String? msg}) result =
        await localBlockRuleService.removeLocalBlockRulesByGroupId(groupId);
    if (!result.success) {
