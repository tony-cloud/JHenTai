...
class LoginPageLogic extends GetxController {
  ...
  Future<void> _handleCookieLogin() async {
    ...
    if (state.igneous != null &&
        state.igneous != '' &&
        state.igneous != 'null' &&
        state.igneous != 'mystery' &&
        state.igneous != 'deleted') {
      ehRequest.storeEHCookies([
    ...
    try {
      ...
      userInfo = await ehRequest.requestForum(
          int.parse(state.ipbMemberId!), EHSpiderParser.forumPage2UserInfo);
    } on DioException catch (e) {
      log.error('loginFail'.tr, e.errorMsg);
  ...
  Future<void> _onDesktopPageStarted(Webview webview, String url) async {
    ...
    try {
      ...
      int ipbMemberId =
          int.parse(cookies.firstWhere((cookie) => cookie.name == 'ipb_member_id').value);
      String ipbPassHash = cookies.firstWhere((cookie) => cookie.name == 'ipb_pass_hash').value;

      /// temporary name
      userSetting.saveUserInfo(
          userName: 'EHUser'.tr, ipbMemberId: ipbMemberId, ipbPassHash: ipbPassHash);
      ...
      untilRoute(
        ...
        predicate: (route) =>
            route.settings.name == Routes.settingAccount ||
      ...
      Map<String, String?>? userInfo =
          await ehRequest.requestForum(ipbMemberId, EHSpiderParser.forumPage2UserInfo);
      userSetting.saveUserNameAndAvatarAndNickName(
        userName: userInfo!['userName']!,
        avatarImgUrl: userInfo['avatarImgUrl'],
        nickName: userInfo['nickName']!,
  ...
  Future<void> _onMobilePageStarted(String url, WebViewController controller) async {
    ...
    try {
      ...
      untilRoute(
        ...
        predicate: (route) =>
            route.settings.name == Routes.settingAccount || route.settings.name == Routes.home,
      );
