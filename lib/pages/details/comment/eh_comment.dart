...
class _EHCommentState extends State<EHComment> {
  ...
  Future<void> _handleBlockUser(BuildContext context) async {
    bool? result =
        await showDialog(context: context, builder: (_) => EHDialog(title: '${'blockUser'.tr}?'));
    if (result == true) {
...
class _EHCommentTextBody extends StatelessWidget {
  ...
  InlineSpan buildTag(BuildContext context, dom.Node node) {
    ...
    if (node.localName == 'img') {
      ...
      if (inDetailPage) {
        return TextSpan(
            text: '[${'image'.tr}]  ', style: const TextStyle(color: UIConfig.commentLinkColor));
      }
            ...
            .map(
              (childTag) => Text.rich(
                ...
                style: const TextStyle(
                    color: UIConfig.commentLinkColor, fontSize: UIConfig.commentLinkFontSize),
              ),
            )
            .toList(),
      );
    ...
    if (match.start == 0) {
      return TextSpan(
        ...
        recognizer: inDetailPage
            ? null
            : (TapGestureRecognizer()..onTap = () => _handleTapUrl(match.group(0)!)),
  ...
  TextStyle? _parseTextStyle(dom.Element node) {
    ...
    return TextStyle(
      color: styleMap['color'] == null
          ? null
          : Color(int.parse(styleMap['color']!.substring(1), radix: 16) + 0xFF000000),
    ...
    return Row(
      children: [
        ...
        if (score.isNotEmpty && !widget.fromMe && !widget.disableButtons) ...[
          LikeButton(
            size: widget.inDetailPage
                ? UIConfig.commentButtonSizeInDetailPage
                : UIConfig.commentButtonSizeInCommentPage,
            likeBuilder: (_) => Icon(
              Icons.thumb_up,
              size: UIConfig.commentButtonSizeInDetailPage,
              color: widget.votedUp
                  ? UIConfig.commentButtonVotedColor(context)
                  : UIConfig.commentButtonColor(context),
            ),
        ...
        GestureDetector(
          onTap: () => score.isEmpty
              ? null
              : Get.dialog(EHCommentScoreDetailsDialog(scoreDetails: widget.scoreDetails)),
            ...
            child: Align(
              ...
              child: score.isEmpty
                  ? Text(
                      'uploader'.tr,
                      style: TextStyle(
                        fontSize: widget.inDetailPage
                            ? UIConfig.commentScoreSizeInDetailPage
                            : UIConfig.commentScoreSizeInCommentPage,
                        color: UIConfig.commentFooterTextColor(context),
                      ),
                    )
                  : Text(
