...
class GalleryListDownloadPage extends StatelessWidget
  ...
  Widget _buildSuperResolutionLabel(BuildContext context, GalleryDownloadedData gallery) {
    return GetBuilder<srs.SuperResolutionService>(
      ...
      builder: (_) {
        srs.SuperResolutionInfo? superResolutionInfo =
            superResolutionService.get(gallery.gid, srs.SuperResolutionType.gallery);
        ...
        return Container(
          ...
          decoration: BoxDecoration(
            borderRadius: superResolutionInfo.status == srs.SuperResolutionStatus.success
                ? null
                : BorderRadius.circular(4),
            border: Border.all(color: UIConfig.resumePauseButtonColor(context)),
            shape: superResolutionInfo.status == srs.SuperResolutionStatus.success
                ? BoxShape.circle
                : BoxShape.rectangle,
          ),
          child: Text(
            ...
            style: TextStyle(
              fontSize: 9,
              color: UIConfig.resumePauseButtonColor(context),
              decoration: superResolutionInfo.status == srs.SuperResolutionStatus.paused
  ...
  Widget _buildPriority(BuildContext context, GalleryDownloadedData gallery) {
    ...
    switch (priority) {
      case 1:
        return Text('①',
                style: TextStyle(
                    color: UIConfig.resumePauseButtonColor(context), fontWeight: FontWeight.bold))
  ...
  Widget _buildButton(BuildContext context, GalleryDownloadedData gallery) {
    return GetBuilder<GalleryDownloadService>(
      ...
      builder: (_) {
        DownloadStatus downloadStatus = logic
    ...
    return GetBuilder<GalleryDownloadService>(
      ...
      builder: (_) {
        GalleryDownloadProgress downloadProgress =
            logic.downloadService.galleryDownloadInfos[gallery.gid]!.downloadProgress;
        GalleryDownloadSpeedComputer speedComputer =
            logic.downloadService.galleryDownloadInfos[gallery.gid]!.speedComputer;
        return Column(
          children: [
            Row(
              children: [
