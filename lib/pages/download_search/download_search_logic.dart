...
import 'package:jhentai/utils/table.dart' as t;
import 'package:jhentai/widget/eh_alert_dialog.dart';
import 'package:jhentai/widget/eh_download_dialog.dart';
import 'package:jhentai/pages/download_search/download_search_state.dart';
...
class DownloadSearchLogic extends GetxController with UpdateGlobalGalleryStatusLogicMixin {
        ...
        .map(
          (g) => GallerySearchVO(
            ...
            tags: tagDataString2TagDataList(g.tags)
                .map((tagData) =>
                    translatedTagDataTable.get(tagData.namespace, tagData.key) ?? tagData)
                .toList(),
    ...
    List<ArchiveSearchVO> archives = archiveDownloadService.archives.map((a) {
      return ArchiveSearchVO(
        ...
        tags: tagDataString2TagDataList(a.tags)
            .map((tagData) => translatedTagDataTable.get(tagData.namespace, tagData.key) ?? tagData)
            .toList(),
    ...
    if (state.searchType == DownloadSearchConfigTypeEnum.regex) {
      ...
      state.gallerys = gallerys.where((g) {
        ...
        for (TagData tagData in g.tags) {
          ...
          if (tagData.translatedNamespace != null &&
              ...
              regExp.hasMatch('${tagData.translatedNamespace}:${tagData.tagName}')) {
            return true;
          }
        }
        return false;
      ...
      state.gallerys = gallerys.where((g) {
        ...
        for (TagData tagData in g.tags) {
          ...
          if (tagData.translatedNamespace != null &&
              tagData.tagName != null &&
              '${tagData.translatedNamespace}:${tagData.tagName}'.contains(value)) {
            return true;
          }
        }
        return false;
  ...
  Future<void> toggleSearchType() async {
    ...
    await localConfigService.write(
        configKey: ConfigEnum.downloadSearchPageType, value: state.searchType.code.toString());
  }
