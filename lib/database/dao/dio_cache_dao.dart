...
class DioCacheDao {
  static Future<DioCacheData?> selectByCacheKey(String cacheKey) {
    return (appDb.select(appDb.dioCache)..where((tbl) => tbl.cacheKey.equals(cacheKey)))
        .getSingleOrNull();
  }
  ...
  static Future<int> deleteCacheByDate(DateTime date) {
    return (appDb.delete(appDb.dioCache)..where((tbl) => tbl.expireDate.isSmallerThanValue(date)))
        .go();
  }
